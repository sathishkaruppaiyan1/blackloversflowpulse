...
const buildPendingProducts = (orders: WooCommerceOrder[]): PendingProductGroup[] => {
  const map = new Map<string, PendingProductGroup>();
  ...
  for (const order of cleanOrders) {
    for (const item of order.line_items || []) {
      ...
      const key = `${cleanName.toLowerCase()}||${color.toLowerCase()}||${size.toLowerCase()}`;
...
const ProductsPendingPage = () => {
  ...
  const handlePrint = () => {
    ...
    const cardsHtml = filteredProducts.map(p => {
      const swatch = colorSwatchStyle(p.color).background as string;
      const imgHtml = p.image
        ? `<img src="${p.image}" alt="" />`
        : `<div class="ph"></div>`;
      const colorChip = p.color
        ? `<span class="chip"><span class="dot" style="background:${swatch}"></span>${p.color}</span>`
        : '';
      const sizeChip = p.size ? `<span class="chip size">${p.size}</span>` : '';
      return `
        <div class="card">
          <div class="img">${imgHtml}</div>
          <div class="body">
            <div class="name">${p.name}</div>
            <div class="chips">${colorChip}${sizeChip}</div>
            <div class="meta">
                  ...
                  <div key={p.key} className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
                    ...
                    <div className="p-3 flex flex-col gap-2 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {p.size && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-700 font-semibold">
                            {p.size}
                          </span>
                        )}
                      </div>
